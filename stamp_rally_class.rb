...
class Searcher                                                   #探索するクラス
  ...
  def start_island                                                     #最初の島を選ぶ関数
    ...
    @sum_island.times do |i|                                           #全ての島に適用
      if number_of_adjacent_island < @adjacent_island[i].size          #隣接数が一番多い島を選ぶ
        number_of_adjacent_island = @adjacent_island[i].size           #隣接数の記憶
...
class Preparation                                      #探索の準備をするクラス（1文字ずつにわける、型の変換、枝切り）
  ...
  def pruning                                                      #枝切りの関数　枝切り：隣接数1の島を消して行き止まりに達する可能性を減らす
    ...
    while @delete != []                                            #隣接数が1の島が無くなるまで繰り返す
      @sum_island.times do |i|                                     #全ての島に適用
        ...
        delete_isolated_island(@adjacent_island[i], i, delete_tmp)
      end
  ...
  def delete_isolated_island(adjacent_island, i, array)
...
preparation = Preparation.new(sum_island, adjacent_island)
preparation.format                                                 #1文字ずつに分ける&枝切り準備
preparation.pruning                                                #枝切り
